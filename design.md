...
版本：2026-09-04（重构基线，F5 收口）
状态：本文替代旧版 design.md。旧版“单课程、文字优先、2/3/7/21 天、可选 LightRAG 主线”的设计不再是目标架构；当前代码覆盖图片主链 A–F5，语义/图 RAG 等仍后置。
...
1. **收录**：多图选择、缩略图/原图、顺序和角色、科目候选；上传后原图立即保存，资料逐份自动增强，题目自动顺序分析和匹配，按钮仍可用于重试。
...
模型、协议和主/回退配置可通过首页设置区或 `/api/settings/model` 保存，读取时只返回 key 是否已配置；没有保存时继续兼容 `LLM_*`/`LLM_FALLBACK_*` 环境变量。资料上传和图片 intake 都在保存后自动推进现有解析/分析链。数学↔专业课按上述主科优先、未分类其次、另一科最后的规则轻量补召回，英语和政治不自动跨科。分析完成时优先给用户两个业务结果标签：“匹配题目完成”或“模型解答完成”；内部仍可显示“已保存 / 分析中 / 草稿 / 未完整 / 已确认”。不出现“校验失败所以不能继续”的阻断文案。
...
GET   /api/settings/model                # 读取模型设置（只返回 key 是否已配置）
PATCH /api/settings/model                # 保存 primary/fallback 模型设置
GET   /api/sources?limit=30              # 最近资料及解析状态
...
当前实现保持轻量：未指定角色时不静默采用过程/答案图，错题详情可补选角色和顺序；题面与过程混在同一张图时，回测会明确提示而不伪装成完全隔离。候选选择已统一处理并保存纯答案文本；确认缺少诊断不再阻断。确认后追加并标记为 `reference` 的已保存图片会作为新增 grading 输入同步到正式快照，不改写历史 Attempt。P0 fixture 带有“演示数据”标识，旧文字链在投影中带“演示/兼容”标识。当前 FTS 仍是轻量片段/整段召回，不宣称已经解决相似题语义匹配；数学↔专业课仅按主科优先、未分类其次、另一科最后做轻量补召回，英语和政治不跨科。
...
沿用现有 `ReviewTask` / `Question`，不新增事实表或技术门禁：首页队列只展示 open 任务，按到期优先排序并显示科目、轮次、原因和状态；点击任一任务可直接打开对应错题详情并开始闭卷重做。开始闭卷重做后仍只展示题面，提交路径和固定 +3/+7/+10/+14 排程不变。
...
- F1 真实使用流程：首页可保存 primary/fallback 模型设置；资料和图片上传后自动顺序增强/分析/匹配；数学和专业课按轻量跨科规则补召回，确认后立即进入错题详情和固定回测排程。
- F5 回测队列收口：首页显示全部 open 任务并按到期优先排序，队列行可直接打开对应错题并开始闭卷重做；沿用既有 ReviewTask/Question，不新增表或门禁。
