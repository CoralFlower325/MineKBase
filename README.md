...
- 正式错题本支持按具体课程、科目、章节、知识点、题型和错误类型做轻量精确筛选；列表与知识导航直接显示三类错误或“待确认”；两个 Markdown 导出入口沿用当前筛选条件；错题详情可以重新绑定或解除同课程个人知识节点，历史 Attempt 快照不变；不带筛选条件时仍显示分类为空的题目，408 与信号与系统等同属专业课的课程不会混在一起。
